    ...
    return (
        <Card sx={{ maxWidth: '400px', minWidth: isMobile ? '300px' : '400px', marginTop: '18px' }}>
            <CardContent sx={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                {isLoading ? <Skeleton variant="rounded" height={20} width="70%" animation="wave" /> :
                ...
                {isLoading ? <Skeleton variant="rounded" height={20} animation="wave" /> :
                    ...
                    <Box sx={{ display: 'flex', gap: '10px' }}>
                        <Typography>
                            {t(statsFields.branches_count.name)} -
                        </Typography>
                        <Typography color={'green'}>
                            {statsFields.branches_count.value}
                        </Typography>
                    </Box>
                }
                {isLoading ? <Skeleton variant="rounded" height={20} animation="wave" /> :
                    ...
                    <Box sx={{ display: 'flex', gap: '10px' }}>
                        <Typography>
                            {t(statsFields.courses_count.name)} -
                        </Typography>
                        <Typography color={'green'}>
                            {statsFields.courses_count.value}
                        </Typography>
                    </Box>
                }
                {isLoading ? <Skeleton variant="rounded" height={20} animation="wave" /> :
                    ...
                    <Box sx={{ display: 'flex', gap: '10px' }}>
                        <Typography>
                            {t(statsFields.employees_count.name)} -
                        </Typography>
                        <Typography color={'green'}>
                            {statsFields.employees_count.value}
                        </Typography>
                    </Box>
                }
                {isLoading ? <Skeleton variant="rounded" height={20} animation="wave" /> :
                    <Box sx={{ display: 'flex', gap: '10px' }}>
                        <Typography>
                            {t(statsFields.groups_count.name)} -
                        </Typography>
                        <Typography color={'green'}>
                            {statsFields.groups_count.value}
                        </Typography>
                    </Box>
                }
                {isLoading ? <Skeleton variant="rounded" height={20} animation="wave" /> :
                    <Box sx={{ display: 'flex', gap: '10px' }}>
                        <Typography>
                            {t(statsFields.rooms_count.name)} -
                        </Typography>
                        <Typography color={'green'}>
                            {statsFields.rooms_count.value}
                        </Typography>
                    </Box>
                }
                {isLoading ? <Skeleton variant="rounded" height={20} animation="wave" /> :
