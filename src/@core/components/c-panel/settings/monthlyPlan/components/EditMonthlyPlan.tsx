...
import { fetchTariffs, handleEditMonthlyPlan, updateTariff } from "src/store/apps/c-panel";
...
export default function EditMonthlyPlan() {
    ...
    const { editedMonthlyPlan } = useAppSelector(state => state.cPanelSlice)
    const dispatch = useAppDispatch()

    useEffect(() => {
        if (editedMonthlyPlan) {
            formik.setValues(editedMonthlyPlan)
        }
    }, [editedMonthlyPlan])
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values: Partial<TariffType>) => {
            setLoading(true)
            const response = await dispatch(updateTariff(values))
    ...
    const handleClose = () => {
        formik.resetForm()
        dispatch(handleEditMonthlyPlan(null))
    }

    return (
        <Drawer open={!!editedMonthlyPlan} hideBackdrop anchor='right' variant='temporary' >
