...
import { fetchSMSTariffs, handleEditSMSTariff, updateSMSTariff } from "src/store/apps/c-panel";
import { SMSTariff } from "src/types/apps/cpanelTypes";
import * as Yup from "yup";

export default function EditMonthlyPlan() {
    const { t } = useTranslation()
    const [isLoading, setLoading] = useState(false)
    const { editedSMSTariff } = useAppSelector(state => state.cPanelSlice)
    const dispatch = useAppDispatch()

    useEffect(() => {
        if (editedSMSTariff) {
            formik.setValues(editedSMSTariff)
        }
    }, [editedSMSTariff])

    const validationSchema = Yup.object({
        amount: Yup.string().nullable().required(t("SMS tarif summasini kiriting") as string),
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values: Partial<SMSTariff>) => {
            setLoading(true)
            const response = await dispatch(updateSMSTariff(values))
    ...
    const handleClose = () => {
        formik.resetForm()
        dispatch(handleEditSMSTariff(null))
    }

    return (
        <Drawer open={!!editedSMSTariff} hideBackdrop anchor='right' variant='temporary' >
                ...
                <Box width={'100%'}>
                    <form onSubmit={formik.handleSubmit} style={{ display: 'flex', flexDirection: 'column', width: '100%', alignItems: 'baseline', padding: '20px 10px', gap: '10px' }}>
                        <FormControl sx={{ width: '100%' }}>
                            ...
                            />
                            <FormHelperText error={!!formik.errors.sms_count && !!formik.touched.sms_count}>{!!formik.errors.sms_count && !!formik.touched.sms_count && formik.errors.sms_count}</FormHelperText>
                        </FormControl>
                        <FormControl sx={{ width: '100%' }}>
                            <TextField
                                name='amount'
