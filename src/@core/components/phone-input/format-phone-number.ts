export const formatPhoneNumber = (value: any) => {
  ...
  const formattedPart = `(${phoneNumber.slice(0, 2)}) ${phoneNumber.slice(2, 5)}-${phoneNumber.slice(
    5,
    7
...
export const reversePhone = (value: string) => {
  ...
  return `+998${
    withoutPrefix.replace(/[^\d]/g, '').length <= 9
      ? withoutPrefix.replace(/[^\d]/g, '')
      : withoutPrefix.replace(/[^\d]/g, '').split('').slice(0, 9).join('')
  }`
}
