            ...
            try {
                ...
                if (response.status == 201) {
                    ...
                    if (query.month && query?.id) {
                        await dispatch(getAttendance({ date: `${query?.year || new Date().getFullYear()}-${getMontNumber(query.month)}`, group: query?.id, queryString: '' }))
                    }
            ...
            <DialogContent>
                <form style={{ marginTop: 10 }} onSubmit={formik.handleSubmit} >
                    <FormControl sx={{ maxWidth: '100%', mb: 3 }} fullWidth>
                        <InputLabel error={!!formik.errors.new_group && formik.touched.new_group} size='small' id='demo-simple-select-outlined-label'>Yangi guruh</InputLabel>
                        <Select
