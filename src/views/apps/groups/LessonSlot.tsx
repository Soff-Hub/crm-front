  ...
  >
    <Box
      sx={{
        ...
        borderRadius: '8px',
        backgroundColor: item.color.split(',')[0] || '#FFF',
        width: '100%',
        height: '100%',
        cursor: 'pointer',
        padding: '2px 6px',
    ...
    >
      <Typography sx={{ color: item.color.split(',')[1] || '#000', fontSize: '10px' }}>
        {hourFormatter(item.start_at)} - {hourFormatter(item.end_at)} / {item.name}
      </Typography>
      <Typography sx={{ color: item.color.split(',')[1] || '#000', fontSize: '10px' }}>{item.teacher_name}</Typography>
    </Box>
  </Box>
)
