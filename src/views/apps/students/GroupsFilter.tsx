      ...
      <form id='mobile-filter-form'>
        <Box display={'flex'} gap={2} flexDirection={'column'} paddingTop={isMobile ? 3 : 0} rowGap={isMobile ? 4 : 0}>
          ...
          <DatePicker
            ...
            popperPlacement={'top'}
            customInput={<CustomInput label='Sana' start={startDate as Date | number} end={endDate as Date | number} />}
          />
        </Box>
      </form>
    )
    ...
    <Box display={'flex'} gap={2} flexWrap={'nowrap'}>
      ...
      <FormControl sx={{ maxWidth: 180, width: '100%' }}>
        ...
        >
          ...
          <MenuItem value={10}>{t('Juft kunlari')}</MenuItem>
          <MenuItem value={20}>{t('Toq kunlari')}</MenuItem>
          <MenuItem value={30}>{t('Har kuni')}</MenuItem>
          <MenuItem value={30}>{t('Boshqa')}</MenuItem>
        </Select>
      </FormControl>
      <DatePicker
        selectsRange
        ...
        customInput={
          <CustomInput label='Sana' end={endDateRange as Date | number} start={startDateRange as Date | number} />
        }
      />
    </Box>
  )
