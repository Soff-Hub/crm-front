    ...
    <Box className='demo-space-y' sx={{ display: 'flex', flexDirection: 'column' }}>
      {loading ? (
        ...
        newData.map((_: any, index: number) => (
          <Link
            href={`/groups/view/security/?id=${_.id}&month=${getMontName(null)}`}
            key={index}
              ...
              <Card sx={{ width: isMobile ? '100%' : '50%' }}>
                <CardContent sx={{ display: 'flex', justifyContent: 'space-between', margin: '0', py: '6px' }}>
                  <Chip
                    ...
                    size='small'
                    variant='outlined'
                    color={_.status === 'active' ? 'success' : _.status === 'archived' ? 'error' : 'warning'}
                  />
                </CardContent>
                ...
                <CardContent sx={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: ' flex-end', gap: '5px' }}>
                    <Typography sx={{ fontSize: '12px' }}>
                      {t('Ochilgan sana')}: {_.start_date}
                    </Typography>
                    <Typography sx={{ fontSize: '12px' }}>
                      {t(`${t(getLessonDays(_.week_days))}`)} / {_.start_at.split(':').splice(0, 2).join(':')} -{' '}
                      {_.end_at.split(':').splice(0, 2).join(':')}
                    </Typography>
