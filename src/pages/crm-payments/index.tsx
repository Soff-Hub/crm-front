...
import {
  fetchClientSideTariffs,
  fetchCRMPayments,
  fetchSMSTariffs,
  handleOpenClientModal,
  handleOpenClientSMSModal,
  updateClientParams
} from 'src/store/apps/c-panel'
...
export default function PaymentsList() {
  ...
  const column: customTableDataProps[] = [
      ...
      render: (item: any) => {
        ...
        if (found?.tariff) {
          return (
            <div>
              <span style={{ color: '#22d3ee', marginRight: '5px' }}>
                {found?.tariff_data?.month_count} {t('oylik')}
              </span>
              <span style={{ color: '#f59e0b', marginRight: '5px' }}>
                ({formatCurrency(found?.tariff_data?.amount)} so'm)
              </span>
              <span style={{ color: '#84cc16' }}>
                ({found?.tariff_data?.min_count}-{found?.tariff_data?.max_count} {t("ta o'quvchi")})
              </span>
            </div>
          )
        } else {
          return (
            <div>
              <span style={{ color: '#84cc16' }}>
                ({found?.sms_data?.sms_count} {t('SMS')})
              </span>{' '}
              <span style={{ color: '#f59e0b', marginRight: '5px' }}>
                ({formatCurrency(found?.sms_data?.amount)} so'm)
              </span>
            </div>
          )
        }
    ...
    },
    {
      ...
      render: status =>
        status == 'moderation' ? (
          <Chip label={t('Tekshirilmoqda')} size='small' />
        ) : status == 'approved' ? (
        ...
        >
          ...
          <Stack display={isMobile ? 'grid' : 'flex'} direction={'row'} alignItems={'center'} gap='15px'>
            <Chip
              ...
              size='small'
              label={
                clientOwnPayments?.is_demo
                  ? `${t('Demo tugash vaqti')} : ${clientOwnPayments?.expiration_date}`
                  : `${t("Keyingi to'lov vaqti")} : ${clientOwnPayments?.expiration_date}`
              }
              color='error'
              variant='outlined'
            />
            <Chip
              ...
              icon={
                <Tooltip
                  arrow
                  title="Platforma uchun to'lov hamda qo'shimcha SMS paket olish  uchun to'lovlarni o'ng tomonda joylashgan To'lov qilish yoki SMS paket olish tugmasini bosish orqali markazga mos tushuvchi tariflardan birini tanlab ushbu karta raqamga qilingan to'lov chekini yuboring (Zufarbek Abdurahmonov)"
                >
              ...
              label='9860 0201 1188 1916'
              color='warning'
              variant='outlined'
            />
            <Button
              size='small'
              onClick={() => dispatch(handleOpenClientSMSModal(true))}
              color='primary'
              variant='outlined'
            >
            ...
            <Button
              ...
              variant='contained'
            >
              {t("Tarif uchun to'lov")}
            </Button>
          </Stack>
        </Box>
        <DataTable loading={isGettingOwnPayments} columns={column} data={clientOwnPayments?.results || []} />
        {clientOwnPayments && clientOwnPayments?.count > 10 && !isGettingOwnPayments && (
          <Pagination
