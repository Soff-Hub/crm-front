...
const AppCalendar = () => {
  ...
  const [tabValue, setTabValue] = useState<string>(
    currentWeek === 'mon' || currentWeek === 'wed' || currentWeek === 'fri' ? '2' : '1'
  )
  const [open, setOpen] = useState<null | 'week'>(null)
  const [weeks, setWeeks] = useState<any>(
    query?.weeks
      ? typeof query.weeks === 'string'
        ? query.weeks.split(',')
        : query.weeks
            ...
            <table border={0}>
              <tbody>
                <tr>
                  ...
                  <td>
                    <Box sx={{ display: 'flex' }}>
                      {generateTimeSlots(startTime, endTime).map((el: string) => (
                        <Box
                          key={el}
                          sx={{
                            width: '50px',
                      ...
                      <Box sx={{ display: 'flex', position: 'relative', marginLeft: '25px' }}>
                        ...
                        {lesson.lessons.map((item: any) => (
                          <Box
                            ...
                            key={item.id}
                            sx={{
                              width: `${generateTimeSlots(item.start_at, item.end_at).length * 50}px`,
                              height: '45px',
                              position: 'absolute',
                              padding: '5px',
                              left: `${
                                generateTimeSlots(startTime, endTime).findIndex(
                                  el => el === generateTimeSlots(item.start_at, item.end_at)[0]
                                ) * 50
                              }px`
                            }}
                          >
                ...
                <FormControl key={week}>
                  ...
                  >
                    ...
                    <Checkbox
                      ...
                      onClick={() =>
                        setWeeks((c: any) => (c.includes(week) ? c.filter((dd: any) => dd !== week) : [...c, week]))
                      }
                    />
                  </label>
                </FormControl>
